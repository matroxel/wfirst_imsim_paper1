...
\newcommand{\chris}[1]{\textcolor{green}{CH: #1}}
...
\rachel{Consider replacing lengthy section title with something like ``WFIRST background''?} \chris{Agreed; changed}
...
\rachel{I've noticed that people who are new to this process often interpret terminology in surprising ways.  Recommendation: explicitly state that known biases will be characterized and remove, and requirements are on the {\em residual systematic uncertainty}.  I've noticed ``systematic error'' is sometimes interpreted as the combination of the bias and its uncertainty, rather than just its uncertainty.  Even if you want to say ``systematic error'' throughout, rather than changing all instances to ``residual systematic uncertaintay'' it would be good to clarify this once.} \verify{See my addition in second paragraph to address this} \chris{Accepted new version}
...
\rachel{Connecting back to my point about removing biases versus placing requirements on residual uncertainties, I would suggest referring to requirements on $\delta m$ and $\delta c$ to emphasize that known sources of shear calibration bias will be corrected.} \chris{slight re-phrasing, see if we like this}

\subsection{Mapping from wavefront error to galaxy ellipticities -- analytic approach}
\label{ss:dedZ}

The requirements based on $Z$ are described in terms of shear systematics, but in order to be useful for engineering, we need to write a requirement in terms of wavefront errors. The key step to doing this is to write the derivative of the observed shear $\gamma_{\rm obs}$ with respect to the wavefront error $\psi_j$ (where $j$ denotes a Zernike mode). Because the PSF size and ellipticity are quadratic rather than linear in the wavefront error, it is necessary to take a quadratic expansion; then $\partial\gamma_{{\rm obs},i}/\partial\psi_j$ is linear in the static wavefront error $\psi_j$ \citep{2010SPIE.7731E..1EN}. Our approach is to use symmetries to categorize the possible quadratic terms, which gives 4 independent coefficients if we take the first 11 Zernike modes (up through spherical aberration). We can then use a suite of simple simulations to determine the 4 coefficients. Then -- given a limit on the static wavefront error $\lVert{\boldsymbol\psi}\rVert \le 92$ nm rms \rachel{seems like it'd be good to link to some document where this is derived or set, if it lives somewhere public?} \chris{Doesn't live somewhere public yet, working on that.}, set to achieve diffraction-limited imaging in $J$-band -- we can analytically search the space of possible static wavefront errors and find the maximum possible $|\partial\gamma_{{\rm obs},i}/\partial\psi_j|$ (units: nm$^{-1}$). This worst-case sensitivity can be used to set requirements on knowledge of the WFIRST wavefront.

\rachel{For non-weak lensers, it may be necessary to spell out that the requirement fundamentally derives from the fact that we use the PSF model to correct the observed galaxy shapes for the convolution with the PSF, and if we do not know the wavefront well enough, our PSF model will be sufficiently wrong as to induce systematic uncertainty in the shear.  (This is obvious to us, but it's another point that I find gives rise to confusion.)} \verify{see my addition in final paragraph of subsection below to address this} \chris{Approve change.}
...
The partial derivatives $|\partial e/\partial\psi|$ of the ellipticity with respect to wavefront should be less than $\lVert {\boldsymbol\Lambda} \rVert \lVert {\boldsymbol\psi} \rVert$ (where ${\boldsymbol\Lambda}$ is the analytically derived matrix defined in Eq.~\ref{eq:LambdaDef} of Appendix~\ref{app:psf-stability}), which is $8.98\times 10^{-4}\,$nm$^{-1}$ for the H158-band (this is an RSS of the two ellipticity components). For the partial derivatives of the ellipticity with respect to the second moments of the jitter pattern, $|\partial e/\partial\psi|$ should be less than $K_{\theta\theta}$ (where $K_{\theta\theta}$ is the analytically derived sensitivity to jitter; see Eqs.~\ref{eq:K1t},\ref{eq:K2t} of Appendix~\ref{app:psf-stability}), which is $1.38\times 10^{-5}$\,mas$^{-2}$ for the H158-band. In the numerical results presented in Table~\ref{table:partials}, the largest partial derivatives are $4.5\times 10^{-4}\,$nm$^{-1}$ (for wavefront errors) and $1.45\times 10^{-5}$\,mas$^{-2}$ (for jitter). For the wavefront drift case, this is consistent with the analytic expectations. For the anisotropic jitter case, the sensitivity $|\partial e/\partial\psi|$ determined from the simulations is $5\pm 2$\%\ larger than the analytic bound (we would expect a sensitivity equal to the analytic bound since the AniJitter run is the worst-case jitter pattern: the anisotropy is in the $e_1$ component for every exposure). The $5\pm 2$\% difference may simply represent the approximations made in the analytic calculation (e.g., no treatment of undersampling/image combination, a different shape measurement algorithm, etc.). \rachel{Are the analytic expectations tabulated somewhere that you could refer to, or could they even be put into the table for ease of comparison?} \chris{These are in Appendix B. Should I expand Table~\ref{tab:wfej} (which currently has the jitter coefficients) to include all of the coefficients that we might want to compare and point the reader there?}
