...
Performance details are provided in Appendix \ref{app:performance}. 
...
The specific orientations and gaps between the sensors was updated to match the Cycle 7 \verify{ref} data as part of the development work for this project.
...
\galsim\ is then able to turn these observational parameters into full WCS functions for all 18 SCAs and determine which objects in the scene would be observed by each sensor. \galsim\ automatically accounts for the Jacobian of the WCS transformation when rendering the surface brightness profiles on each sensor's pixels.  
