...
We now proceed to describe requirements and the role of this suite of image simulations in verifying that the requirements flowdown is correct. We begin with a description of weak lensing on WFIRST that emphasizes the issues most closely tied to the image simulations (\S\ref{ss:wlprog}), and a high-level review of the requirements process in a cosmology project (\S\ref{ss:req}). There we describe where in this process we need the mapping between the wavefront error and shear systematics, $\partial c/\partial \psi_j$ and $\partial m/\partial\psi_j$ (where $c$ and $m$ are additive and multiplicative biases on the shear, and $\psi_j$ denotes a Zernike mode of the wavefront error). This mapping was obtained using a simplified analytic model, calibrated by toy simulations, in the Phase A requirements flowdown; this approach is described at a high level in \S\ref{ss:dedZ}, with technical details placed in the appendices. In the rest of this paper, we will use much more advanced image simulations -- the {\sc WFIRST-ImSim} code, built on top of the {\sc GalSim} package -- to estimate $\partial c/\partial \psi_j$ and $\partial m/\partial\psi_j$.

Note that this section is {\em not} a comprehensive introduction to the WFIRST weak lensing program or the requirements flowdown, and many important areas are omitted -- only the aspects directly tied to the image simulations or sensitivity analyses are covered. For a more comprehensive overview, see \verify{[add some cites here]}.

\chris{Do we need a short introduction on Zernikes? or a brief appendix? or simply a textbook reference?}

\subsection{The weak lensing program}
\label{ss:wlprog}
...
\subsubsection{Implementation for additive and multiplicative biases}
\label{ss:add_mult}
...
The top-level requirement on observational systematics can then be broken into contributions for multiplicative biases and additive biases in each angular ($\ell$) bin. The SRD allocated -- in an RSS sense -- 25\% of the top-level budget on $Z$ to additive biases and 25\% to multiplicative biases. This can be done straightforwardly using Eq.~(\ref{eq:zcont}) {\em if} the correct angular and redshift dependence for each template is known and the systematic can be parameterized in terms of a single amplitude parameter $p$. Unfortunately, different underlying sources of error will lead to different angular and redshift dependences, and we must have a way to capture this in the requirements framework. We have chosen a 2-fold approach: for {\em angular} dependences of the additive shear, we have split in-band additive shear biases into 4 bins ($\log_{10}\ell = 1.5-2.0$, $2.0-2.5$, $2.5-3.0$, and $3.0-3.5$), each receiving 25\% of the additive shear budget (i.e., 6.25\% of the top-line budget on $Z$). The additive shear can thus be written as
\begin{equation}
C_\ell^{c_ic_j} = A_0^2 w_i w_j T_\ell,
\end{equation}
where $\ell(\ell+1)T_\ell/(2\pi)$ is a top-hat (constant for $\ell_{\rm min}\le \ell<\ell_{\rm max}$) and with the normalization set to $\sum_\ell (2\ell+1)T_\ell/(4\pi) = 1$. Here $i$ and $j$ represent redshift bin indices and $w_i$ indicates the redshift dependence of the additive shear. In this way, the total additive shear in redshift bin $i$ is $A_0w_i$ RMS per component. We normalize this redshift dependence so that $\max |w_i|=1$ (i.e., the additive shear in the worst redshift bin is $A_0$ RMS per component). The amount of additive shear variance is controlled by the parameter $p = A_0^2$. We have not incorporated an angular dependence for the multiplicative shear, so we set $m_i=w_ip$ for the multiplicative shear case. \verify{[there are some technicalities about this, but I don't think they affect the engineering]}

We handle the {\em redshift} dependence of the systematic errors by first computing requirements for redshift-independent additive or multiplicative systematics ($w_i=1$). We then define a {\em scaling factor} for any other redshift dependence $w_i$:
\begin{equation}
S[{\bf w}] = \frac{Z \,{\rm for\,this\,}w_i}{Z\,{\rm for\,all\,}w_i=1}.
\end{equation}
A systematic that is ``made worse'' by its redshift dependence will have $S>1$, and a systematic that is ``made less serious'' by its redshift dependence will have $S<1$. The WFIRST SRD thus contains requirements on the additive shear bias in each of the 4 bins, and the multiplicative shear bias, but with the caveat that each source of systematic must have its scaling factor computed before comparison to a requirement.\footnote{If truly nothing can be said about the redshift dependence of a systematic, one could use the maximum possible values of the $S$-factor. Because $S[{\bf w}]$ is a positive-definite quadratic function of ${\bf w}$, one can simply search the corners of the $N$-dimensional hypercube $|w_i|<1$. However, we have not run into this situation, since usually if one understands the physics underlying an effect well enough to write a requirement, one can predict how it might scale with redshift.} These limits are shown in Table~\ref{tab:addbands}.

\chris{Due to the small area of our simulations, I don't think this paper should get into checking the $S$-factors in simulations. There is enough content as it is.}

\begin{table*}
\caption{The allocations and bands used for the additive systematic errors (top part of table) and the multiplicative systematic errors. There are $N_{\rm band}=4$ bands ranging over a total signal band from $1.5\le \log_{10} \ell<3.5$. The fraction of the error budget allocated to each band is also indicated, as are the maximum allowed redshift-independent spurious shear ($A_0^{\rm flat}(\alpha)$, RMS per component), and the maximum scaling factors for redshift dependence, $S_{\rm max,\pm}(\alpha)$ and $S_{\rm max,+}(\alpha)$ (the former allowing arbitrary redshift dependence, i.e., any $w_i$ subject to $\max |w_i|=1$, and the latter restricting the redshift dependence to not change sign as a function of redshift, i.e., $0\le w_i\le 1$).
\label{tab:addbands}}
\center{
\begin{tabular}{r|rrc|c|rr}
\hline\hline
\multicolumn7c{Additive shear systematics} \\
Band $\alpha$ & $\ell_{\rm min}(\alpha)$ & $\ell_{\rm max}(\alpha)$ & Allocation $Z(\alpha)$ & Requirement on $A_0(\alpha)$ & $S_{\rm max,\pm}(\alpha)$ & $S_{\rm max,+}(\alpha)$ \\ \hline
~ & ~ & ~ & ~ & for $z$-indep.\ ($w_i=1$) & ~ & ~ \\ \hline
...
\hline
\multicolumn7c{Multiplicative shear systematics} \\
~ & ~ & ~ & Allocation $Z$ & Requirement on $m$ & $S_{\rm max,\pm}(\alpha)$ & $S_{\rm max,+}(\alpha)$ \\ \hline
~ & ~ & ~ & ~ & for $z$-indep.\ ($w_i=1$) & ~ & ~ \\ \hline
~ & ~ & ~ & 0.5000 & ? & ? & ? \\
\hline\hline
\end{tabular}
...
The next step in the hierarchy of the requirements flowdown is to build sub-allocations for the additive and multiplicative shear budgets. The contribution considered here is uncertainty in the wavefront model for WFIRST (there are others, associated with, e.g., detector effects and algorithm issues, but these are outside the scope of this paper).  As noted in \S\ref{ss:req-overview}, this involves two pieces, an understanding of how shear biases depend on the wavefront (what are $\partial c/\partial\psi_j$ and $\partial m/\partial\psi_j$, where $c$ and $m$ are the shear biases, and $\psi_j$ is a Zernike mode of the wavefront) and a requirement on the knowledge of the Zernike modes. The scope of this paper is concerned specifically with the sensitivities $\partial c/\partial\psi_j$ and $\partial m/\partial\psi_j$.

% {\em If} the additive and multiplicative biases were redshift-independent, this would imply an allocation of .
...
\subsection{Mapping from wavefront error to shear systematics -- analytic approach}
\label{ss:dedZ}

\subsubsection{Overview of approach}

Given the limited schedule in preparation for the WFIRST SRR, we used an an analytic approach to estimate the sensitivities $\partial c/\partial X$ and $\partial m/\partial X$, where $X$ is any underlying parameter affecting the PSF. Some of the previous approaches to this problem \verify{[cite]} had been based on second moments of the PSF, however both the galaxy profiles and (from space) the PSF are strongly non-Gaussian and their second moments add in a non-trivial way, so we attempted one additional level of sophistication. The modeling was based on convolving a simple model PSF (the square of the Fourier transform of an annular pupil with the specified wavefront, following ``textbook'' methods \verify{[cite Born \& Wolf]}) with an exponential profile galaxy. This galaxy could be circular, with some half-light radius $r_{\rm eff}$, or it could have an applied shear $\gamma$ to have an axis ratio $e^\gamma:e^{-\gamma}$.\footnote{At higher order, one should take into account that the axis ratio of a sheared object is actually $1+g:1-g$, where $g$ is the reduced shear, but this subtlety should not affect weak lensing PSF determination requirements.} We fit an adaptive second moment Gaussian to the resulting postage stamp \verify{[Bernstein \& Jarvis]}, with no noise, sampling, or detector effects. This process leads to an estimate of the PSF-uncorrected ellipticity components $(e_1^{\rm raw},e_2^{\rm raw})$ for that galaxy and that PSF.

\chris{Move some more details of these models up here.}

The additive shear bias in redshift bin $i$ for component $\mu=1,2$ induced by a knowledge error $\Delta X = X_{\rm true} - X_{\rm model}$ in parameter $X$ is then taken to be
\begin{equation}
c_{i,\mu} = \frac{\langle \partial e_\mu^{\rm raw}/\partial X\rangle}{\langle \partial e_\mu^{\rm raw}/\partial \gamma_\mu \rangle} \Delta X,
\label{eq:cimu}
\end{equation}
where the averages are taken over the population of galaxies in redshift bin $i$ (in this case, their distribution of half-light radii \verify{[make sure we describe this distribution from candels]}). The denominator is essentially a shear responsivity factor, and will be denoted ${\cal R}$.

\chris{Describe how we did this for the multiplicative case.}

\subsubsection{Scanning through wavefront parameter space}

Unfortunately, the additive shear sensitivity matrix depends on the specific realization of the static wavefront, because $\langle \partial e_\mu^{\rm raw}\rangle$ is not a linear function of the Zernike amplitudes ${\boldsymbol\psi}$. Indeed, due to symmetries the possible form of $\langle \partial e_\mu^{\rm raw}\rangle$ is restricted, with the result that $\partial \langle \partial e_\mu^{\rm raw}\rangle/\partial\psi_j$ may be suppressed at zero wavefront error (${\boldsymbol\psi}=0$) and be much larger in the realistic case where ${\boldsymbol\psi}\neq 0$ \citep[e.g.][]{2010SPIE.7731E..1EN}. Therefore we must search the entire space of possible wavefront errors ${\boldsymbol\psi}$ -- bounded by the top-level requirement that $|{\boldsymbol\psi}|<92\,$nm -- to find the place where the operator norm is maximized.

The requirement that the PSF inverts (i.e.\ preserves ellipticity and hence spurious shear) under ${\boldsymbol\psi}\rightarrow-{\boldsymbol\psi}$ implies that $\langle \partial e_\mu^{\rm raw}\rangle$ is an even function of ${\boldsymbol\psi}$ (this statement remains true even for an asymmetric pupil, due e.g.\ to the spider). For a circularly symmetric pupil, we find the further restrictions that
\begin{eqnarray}
\frac{\langle \partial e_1^{\rm raw}\rangle}{\cal R} &=& C_{fa} \psi_{20} \psi_{22} + C_{sa} \psi_{40} \psi_{22}
+ C_{cc} (\psi_{31}^2-\psi_{3-1}^2)
\nonumber \\ && + C_{ct} (\psi_{31}\psi_{33} + \psi_{3-1}\psi_{3-3})
+ ...~{\rm and}
\nonumber \\
\frac{\langle \partial e_2^{\rm raw}\rangle}{\cal R} &=& C_{fa} \psi_{20} \psi_{2-2} + C_{sa} \psi_{40} \psi_{2-2}
+ 2C_{cc} \psi_{31} \psi_{3-1}
\nonumber \\ && + C_{ct} (\psi_{31}\psi_{3-3} - \psi_{3-1}\psi_{33})
+ ...\,,
\end{eqnarray}
...
\chris{Re-write these equations to be in the usual Zernike order!}
\begin{eqnarray}
{\bf M}^{\rm T} &=& \left[ \frac{\langle \partial e_\mu^{\rm raw}\rangle/{\cal R}}{\partial\psi_j} \right]^{\rm T}
\nonumber \\
&=& \left( \begin{array}{cc}
...
\label{eq:TheMatrix}
\end{eqnarray}
(we show the transpose here for ease of display).

\chris{Put analytic $C$ coefficients here.}

For error budgeting purposes, we want to know the maximum possible conversion factor from a change in Zernike modes $\Delta{\boldsymbol\psi}$ to a change in $\langle \partial {\bf e}^{\rm raw}\rangle/{\cal R}$. The maximum such conversion factor is given by the maximum singular value of the matrix ${\bf M}$ (this is the ``operator norm'' $\lVert{\bf M}\rVert$ in Banach space language). We thus write the singular value decomposition ${\bf M} = {\bf UDV}^{\rm T}$, where ${\bf U}$ is a $2\times 2$ orthogonal matrix, ${\bf D}$ has 2 diagonal non-negative entries in non-increasing order ($D_{11}\ge D_{22}$) and is otherwise zeroes (and has dimension $2\times N_{\rm Zern}$), and ${\bf V}$ is $N_{\rm Zern}\times N_{\rm Zern}$. Here ${\bf U}$ is simply a rotation of the shear derivative, and due to circular symmetry can be set to the identity by rotating the entire aberration pattern. Thus without loss of generality we can consider cases where ${\bf U}$ is the identity, and then
\begin{equation}
\lVert{\bf M} \rVert = \sqrt{ \sum_j \left(\frac1{\cal R} \frac{\partial \langle e_1 \rangle}{\partial\psi_j} \right)^2 }
= \sqrt{ ({\boldsymbol\Lambda}{\boldsymbol\psi})^{\rm T} {\boldsymbol\Lambda}{\boldsymbol\psi} }
...
Since $C_{fa}$ is usually dominant, we adopt its redshift dependence to determine the weights $w(z_i)$, with the last bin as the reference bin because it is the most heavily contaminated -- the galaxies are smallest in that bin and the $C$-coefficients are largest. However, the weights $w(z_i)$ obtained from $C_{ct}$ (the next largest coefficient) are only slightly different. The scaling factor in the worst angular bin is then $S=0.49$.

We find the greatest contamination is in the $J$-band, where $\lVert{\boldsymbol\Lambda}\rVert = 1.25\times 10^{-5}\,$nm$^{-2}$. With a limit on the total wavefront error of $|{\boldsymbol\psi}({\boldsymbol\theta})|<\psi_{\rm max} = 92\,$nm, we find that the additive shear per component due to errors in the Zernike modes is
\begin{equation}
\sqrt{\frac12\sum_{\mu=1}^2 c_\mu^2} \le \frac1{\sqrt2} \lVert{\boldsymbol\Lambda}\rVert \psi_{\rm max} |\Delta{\boldsymbol\psi}|
= 8.1\times 10^{-4} \,{\rm nm}^{-1}\, |\Delta{\boldsymbol\psi}|.
\end{equation}
Given that the requirements in Table~\ref{tab:addbands} give a top-level additive shear error of $2.6\times 10^{-4}$ per component (which rescales to $2.6\times 10^{-4}/\sqrt{0.49} = 3.7\times 10^{-4}$ using $S=0.49$), we can see that an error in the wavefront model of 0.46 nm, if in the worst possible configuration, can use up the entire allocation for additive shear.

\subsection{Limitations of the analytical approach}

While this type of analytic flowdown from a shear error budget to PSF properties is standard, it does have some limitations. These include:
\begin{itemize}
\item Interactions of the shape measurement with pixelization, undersampling, and noise are not treated.
\item It does not treat galaxy shape fitting across multiple exposures, or any information on the observing strategy and PSF changes.
\item Because the static wavefront error is not a priori known and the tiling strategy is not used, we do not know how the shear errors are distributed across angular scales.
\item The assumed circular symmetry of the problem is not truly valid in the presence of pixelization, or the correct asymmetric pupil.
\end{itemize}
Image simulations offer a way to address all of these issues. By construction, the galaxy images they produce are pixelized at the native pixel scale of WFIRST and are inherently multi-epoch, with each epoch at the correct observatory roll angle. The asymmetric WFIRST pupil can be used as an input to the simulations. A field-dependent static wavefront model has been provided by the WFIRST Project, and the science teams have defined an example tiling strategy at the individual pointing level. The rest of this paper will describe our image simulation suite ({\sc WFIRST-ImSim}), apply it to a study of the sensitivity of galaxy shear errors to the Zernikes, and compare the results to the analytic forecast.
...
\section{Data vector and covariance model}
\label{sec:datamodel}

\chris{I think the tools used here are basically obsolete, there are several that are better. but they were what was available in $\sim$2014/5 when much of this work was done. I will shorten the section accordingly.}
...
redshift slices, as shown in Table~\ref{tab:dNdz}. In order to ensure
...
to run into systematic error limits. We also turn off astrophysical
systematic errors (e.g.\ intrinsic alignments and baryonic
corrections): we do not want to define a weak requirement on WFIRST
based on the expectation that astrophysical systematics will be large
enough to swamp the observational contribution.
...
\chris{--- END OF CURRENT TEXT --- \\ --- EVERYTHING BELOW HERE IS SCRATCH MATERIAL THAT I INTEND TO PULL UP INTO APPENDICES OR THE MAIN TEXT, OR REMOVE FROM THE FINAL PAPER ---}


\section{Implementation for additive and multiplicative systematics}
...
\chris{removed $S$-factor description}
...
thereof). Exceptions to this policy should be considered in cases
where meeting the original systematic budget becomes a cost or
complexity driver, or is not possible. Most measurement biases fall
into the ``additive'' or ``multiplicative'' forms (see
\S\ref{ss:add_mult}) and will be treated according to the formalism
therein. Other sources of systematic error will be treated in their
...
We need a model to associate high-level requirements for each source
of systematic error (e.g.\ a limit on $Z$, $A_0$, etc.) to low-level
requirements on its source (e.g.\ wavefront drift rate in nm/hr). This
section collects the models used for this flow-down for PSF-related
systematics. Note that none of the models described here anywhere near
a complete description of the WFIRST telescope, instrument, and
pipeline. Rather the intention is to understand the sensitivity of
WFIRST weak lensing to specific types of errors.

\subsubsection{Shape model}
\label{ss:shape}

In order to translate a requirement on spurious shear $c$ into
requirements on lower-level quantities, we need to know how a given
effect -- e.g.\ an error in the PSF model -- affects the shear
measurement. In principle this depends on the shape measurement
algorithm, which presents a difficulty because the final shape
measurement algorithm used for WFIRST has not yet been selected (and
the true ``final'' version may not be set until the analysis
stage). However, most methods of measuring shear have some basic
properties in common -- if e.g.\ the true PSF has greater $e_1$ than
the model (i.e.\ is elongated in the $x$-direction), then the inferred
shear in that region of the sky will also have greater $c$; the
coefficient depends on the exact distribution of galaxy morphologies
and sizes, the weighting of galaxy isophotes, the way in which the PSF
ellipticity varies with radius, etc. Therefore we will consider a
range of methods when we determine the quantity $\partial\gamma_{\rm
obs}(z_i)/\partial X$, where $\gamma_{\rm obs}$ is the measured shear
in a region (and in redshift slice $i$) and $X$ is any quantity on
which we want to set a knowledge requirement. The spurious shear in
bin $i$ is taken to be
\begin{equation}
...
\chris{Finish! Also describe implementation for multiplicative systematics.}

In both the additive and multiplicative cases, to compute the
derivatives relating shear systematics quantities (with requirements
set in \S\ref{ss:add_mult}) to lower-level quantities (knowledge of
$X$) we need a simulation package that generates galaxies, measures
their photometric properties, generates a shear estimate, and repeats
this varying the quantity $X$. For the purpose of Phase A requirements
budgeting, we used a very simple model that enables rapid exploration
...
simplified. In most cases, the intention of these models is {\em not}
to be accurate to the level of a few parts in $10^4$ (which would
require a great deal more sophistication, and has not been achieved by
pure first principles modeling in any telescope used for weak lensing
-- some empirical tuning of the final PSF model, e.g. by principal
components analysis, has always been required). Rather the intention
is to address sensitivities: e.g. how far the focus would need to move
during an exposure before we care; whether wavefront error jitter is
important. As the mission develops, PSF modeling will include many
more effects, and we aim in this document to develop requirements that
can be flowed to individual sub-groups in the mission (e.g. a STOP
analysis should tell us how much of the total PSF error budget is
consumed, without having to put in a new model for interpixel
capacitance).

Just as for shape measurement (\S\ref{ss:shape}), we take a simplified
model here. This model used here takes the Fourier transform of an
...
per axis \cite{2007PASP..119..466B}.\footnote{This was measured on an
H2RG. At the time we had to fix this for Phase A requirements
flow-down, we did not have a measurement on the H4RG.} \chris{The data on the H4RGs show that this was too large, but we are still working on the measurement, will need to figure out what to say here.} Other effects that are either known
to be significant at the level needed for WFIRST weak lensing or are
likely to be significant, such as polarized transfer through the
telescope, angle of incidence variations in mirror reflectivity,
interpixel capacitance in the detectors, etc. are not included. Cases
where they may influence stability requirements in a way not captured
by the simple pupil model are considered separately.

The default model had the spider turned off. The spider has a major
effect on the morphology of the wings of the PSF, producing 12 spikes;
studies of the AFTA pupil in the past have found effects of the spider
on the coefficients herein at the few percent level. The spider
further leads to an asymmetric pupil, i.e.\ with odd-order modes in
the decomposition of the amplitude, but this has no appreciable
effects on the relation of ellipticity to low-order Zernike
modes.\footnote{It is known that an odd-order mode in the phase can
...
effect.} Therefore the spider is {\em not} expected to have a
significant effect on the PSF determination procedures or the
stability requirements. \chris{maybe not necessary to go into detail here, we will probably never update the analytic model to treat the spider}

Studies were carried out with both monochromatic and polychromatic
PSFs. From the perspective of stability requirements, the most
important issue is that while the ellipticity of a {\em monochromatic}
PSF is {\em quadratic} in the wavefront error
\cite{2010SPIE.7731E..1EN}, the ellipticity of a {\em polychromatic}
PSF can contain linear terms in the wavefront error if there is
lateral color, as occurs in WFIRST since the beam intersects the
filter at a nonzero angle of incidence.\footnote{In CCD systems
lateral color can be produced in the detector due to the large mean
free path of red photons in silicon. For WFIRST, using thin
direct-bandgap detectors, this effect is much smaller than the lateral
color induced by the optics.} Thus for small wavefront errors it is
possible that a wavefront stability budget based on the quadratic
dependences of PSF ellipticity (e.g. defocus$\times$astigmatism) and
monochromatic PSFs may be too optimistic. The lateral color mixes with
odd aberrations to produce PSF ellipticity, so a special
``coma$\times$lateral color'' contribution is included.

\chris{Need to re-visit the lateral color issue. We didn't specify a separate requirement on this. but not for this paper, maybe}
...
\hline
$K_{\theta c}$ & 8.11 & 6.43 & 4.91 & $10^{-6}$ mas$^{-1}$ nm$^{-1}$ & 2.8--3.0 & 0.536 \\
$K_{\theta t}$ & 3.59 & 4.14 & 4.46 & $10^{-6}$ mas$^{-1}$ nm$^{-1}$ & 2.8--3.0 & 0.368 \\
\hline
$K_{fa}$ & 11.30 & 9.78 & 8.52 & $10^{-6}$ nm$^{-2}$ & 2.8--3.0 & 0.487 \\
$K_{cc}$ & 3.68 & 2.87 & 2.31 & $10^{-6}$ nm$^{-2}$ & 2.8--3.0 & 0.598 \\
$K_{ct}$ & 5.86 & 5.02 & 4.31 & $10^{-6}$ nm$^{-2}$ & 2.8--3.0 & 0.507 \\
$K_{sa}$ & 4.31 & 3.32 & 2.67 & $10^{-6}$ nm$^{-2}$ & 2.8--3.0 & 0.624 \\
\hline
\end{tabular}
}
\end{table}

The RMS spurious shear per component in J-band, weighted by the
$S$-factor (which accounts for redshift dependence), is then
\begin{equation}
\gamma_{\rm rms}S^{1/2} \le \frac{7.01\times 10^{-6}\,{\rm
nm}^{-1}\,{\rm mas}^{-1}\,\sigma_{\rm los-jitter}\sigma_{\rm
wfe-jitter} + 4.78\times 10^{-6} \,{\rm nm}^{-2}\,\sigma_{\rm
wfe-jitter}^2}{N_{\rm ind}^{1/2}}.
\label{eq:wfej-grms}
\end{equation}
This should be compared to the requirement of $2.60\times
10^{-4}$. The line-of-sight jitter is required to be $\sigma_{\rm
los-jitter}=12$ mas rms per axis, and the observing strategy has two
passes at epochs separated by many months so we take $N_{\rm
ind}=2$. From Eq.~(\ref{eq:wfej-grms}), we find that the entire error
budget is taken up for $\sigma_{\rm wfe-jitter}=3.63$ nm rms.
