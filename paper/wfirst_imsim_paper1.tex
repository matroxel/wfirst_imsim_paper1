...
\newcommand{\rachel}[1]{\textcolor{magenta}{RM: #1}}
...
$^{5}$McWilliams Center for Cosmology, Department of Physics, Carnegie Mellon University, Pittsburgh, Pennsylvania 15213, USA\\
$^{6}$Jet Propulsion Laboratory, California Institute of Technology, Pasadena, CA 91109, USA\\
$^{7}$IPAC, Mail Code 314-6, California Institute of Technology, 1200 East California Boulevard, Pasadena, CA 91125, USA\\
$^{8}$Cosmic Dawn centre (DAWN), Niels Bohr Institute, University of Copenhagen, Juliane Maries vej 30, DK-2100 Copenhagen, Denmark\\
...
\begin{abstract}
The Wide-Field InfraRed Survey Telescope (WFIRST) mission, expected to launch in the mid-2020s, and its weak lensing program is designed to enable unprecedented systematics control in photometric measurements, including shear recovery, point-spread function (PSF) correction, and photometric calibration. This will enable exquisite weak lensing science and allow us to confront an uncertain cosmological landscape after the initial years of observations from other concurrent Stage IV dark energy experiments. This potential requires equally careful planning and requirements validation as the mission prepares to enter its construction phase. We present a suite of image simulations based on GalSim that are used to construct a complex, synthetic WFIRST weak lensing survey that incorporates realistic input galaxies and stars, relevant detector non-idealities, and the current reference five-year WFIRST survey strategy. These simulations are designed to enable empirical validation of the WFIRST analytically-derived weak lensing requirements flowdown and ultimately to provide a basis for integration testing of the weak lensing measurement pipelines as they are defined and developed over the next several years. We study an application of the first of these, by producing a suite of 12 matched image simulations, each representing a different perturbation to the wavefront or image motion model, chosen to reflect a range of potential static and low-and high-frequency time-dependent PSF model errors. We analyze the measured shapes of galaxies from each of these simulations and compare them to a reference, fiducial simulation to infer the response of the shape measurement to each of these modes in the wavefront model. We then compare this to existing analytic flowdown requirements, and find general agreement between the empirically derived response and that predicted by the analytic model.
\end{abstract}
...
\rachel{Some notes on random things I noticed (many minor): (1) there is a mix of ``GalSim''
and the macro \galsim\ or hard-coded {\sc GalSim}, so it would be good to standardize on one of them. (2) MNRAS is going to complain the abstract is beyond their 250-word limit.   (3) References could use an update (e.g., my 2017 review is published).  (4) MNRAS does not permit 'in prep' papers to be cited in the reference list - they should just be mentioned as 'in prep' in the text.}


\section{Introduction}\label{sec:intro}

The nature of dark energy, which drives cosmic acceleration in the Universe \rachel{the accelerated expansion of the Universe?}, remains one of the most fundamental mysteries in physics twenty years after its discovery \citep{riess98,perlmutter99,detf,Frieman:2008sn,weinberg13}. 
...
The Wide-Field InfraRed Survey Telescope (WFIRST)\footnote{\url{http://wfirst.gsfc.nasa.gov}} has been designed to take advantage of all of these probes to study dark energy with unprecedented systematic control \citep{2019arXiv190205569A,2019BAAS...51c.341D}. \rachel{Maybe cite the WFIRST SDT report here, given that it first outlined this design?}
...
% Note correction below: HSC is an instrument, and the survey is "the HSC survey".
...
Precise control of these systematics at the statistical precision offered by current WFIRST mission forecasts \citep{forecast1,forecast2} will enable WFIRST to make crucial contributions to the study of new discoveries made in the early years of LSST and Euclid, and to the resolution of any remaining disagreements between surveys.
...
The simulation incorporates realistic distributions of photometric properties for galaxies and stars; complex analytic galaxy models; a simulated fiducial five year, 2000 sq.~deg.~observation strategy for the survey \rachel{earlier you said 6 sq deg, so is it specifically something about the planned dither strategy that is used here rather than the 2000 sq deg area?}; and realistic detector effects, PSF models, and WCS solutions that match current WFIRST design specifications. 
...
\rachel{Consider replacing lengthy section title with something like ``WFIRST background''?}
...
\rachel{Is the WFIRST SRD not publicly available?}
The current Reference Survey in the WFIRST Science Requirements Document (SRD)\footnote{Document reference number WFIRST-SYS-REQ-0020} envisions shape measurements in 3 filters (J129, H158, and F184), where the PSF is at least half-Nyquist sampled (i.e., pixel size $<\lambda/D$; Nyquist sampling would be $\lambda/2D$). Here F184 is the reddest filter on WFIRST, spanning 1.68--2.00 $\mu$m; it is between ground-based $H$ and $K$, and was chosen based on the thermal constraints from the previously existing telescope hardware that was transferred to the program. Photometric redshift determination requires bluer filters as well. WFIRST itself will do a photometric survey in the Y106 filter since there was no ground-based option that would reach the required depth. Ground-based observations will be required for the $z$ and bluer filters; the primary option for collecting these data will be LSST \citep{2009arXiv0912.0201L}. \rachel{Much more succinct and up-to-date LSST info is in the overview paper, so it would be good to cite it too: https://ui.adsabs.harvard.edu/abs/2019ApJ...873..111I/abstract} The expected imaging depth is 26.9/26.95/26.9/26.25 mag AB in Y106/J129/H158/F184, and the expected depth \rachel{galaxy number density?} is 35 galaxies/arcmin$^2$ (H158-band, the best for shape measurement) or 50 galaxies/arcmin$^2$ (co-added bands). The Reference Survey also includes 10\% of the time devoted to medium-deep fields, which have $10\times$ the exposure time over 1\% of the overall survey area, to calibrate the properties of the source galaxies. % Fixed broken reference within sentence by rearranging.
...
\rachel{I've noticed that people who are new to this process often interpret terminology in surprising ways.  Recommendation: explicitly state that known biases will be characterized and remove, and requirements are on the {\em residual systematic uncertainty}.  I've noticed ``systematic error'' is sometimes interpreted as the combination of the bias and its uncertainty, rather than just its uncertainty.  Even if you want to say ``systematic error'' throughout, rather than changing all instances to ``residual systematic uncertaintay'' it would be good to clarify this once.}
Every precision cosmology project has a requirements process to control both its statistical and systematic errors and ensure that the overall mission can achieve its science objectives. In the case of WFIRST, requirements on the Project (e.g., flight hardware and software or ground system support) were baselined early in the mission (the Science Requirements Document was placed under configuration control in 2018), but requirements on science analyses are more flexible and will be fixed at a later date. The statistical error requirements are usually formulated in terms of survey area, depth and image quality in each filter, cadence (for time-domain programs), etc.; their relation to the science reach of the mission is handled by forecasting tools \citep{forecast1,forecast2}. Systematic error control is much more difficult, and the approach may differ depending on whether a source of systematic error is {\em observational} or {\em astrophysical}. Usually, observational systematics (e.g., PSF calibration for weak lensing) can be budgeted within the systems requirements framework of a large project, whereas astrophysical systematics (e.g., baryonic feedback) are addressed through a combination of nuisance parameters, additional observations, and theory/simulation. These astrophysical systematics are important science team responsibilities but are not part of Project requirements and engineering reviews.

We focus now on the approach to observational systematic errors; our focus is on the WFIRST process, but note that something similar has been done for other large weak lensing programs such as LSST and Euclid \citep{euclid_srd,2016SPIE.9911E..05V,LPM-17,LSE-29}. \rachel{Many of the LSST SRD requirements are in fact {\em not} driven by a quantitative flow-down from science-driven requirements.  (No cosmology forecasts went into it.)  This is one reason why the DESC SRD in some cases identifies more stringent requirements than the LSST SRD.  I would recommend citing the DESC SRD in addition to (or even instead of) the LSST SRD, since it comes closer to a science-driven flowdown as you are describing here.}
...
\rachel{Connecting back to my point about removing biases versus placing requirements on residual uncertainties, I would suggest referring to requirements on $\delta m$ and $\delta c$ to emphasize that known sources of shear calibration bias will be corrected.}

\subsection{Mapping from wavefront error to galaxy ellipticities -- analytic approach}
\label{ss:dedZ}

The requirements based on $Z$ are described in terms of shear systematics, but in order to be useful for engineering, we need to write a requirement in terms of wavefront errors. The key step to doing this is to write the derivative of the observed shear $\gamma_{\rm obs}$ with respect to the wavefront error $\psi_j$ (where $j$ denotes a Zernike mode). Because the PSF size and ellipticity are quadratic rather than linear in the wavefront error, it is necessary to take a quadratic expansion; then $\partial\gamma_{{\rm obs},i}/\partial\psi_j$ is linear in the static wavefront error $\psi_j$ \citep{2010SPIE.7731E..1EN}. Our approach is to use symmetries to categorize the possible quadratic terms, which gives 4 independent coefficients if we take the first 11 Zernike modes (up through spherical aberration). We can then use a suite of simple simulations to determine the 4 coefficients. Then -- given a limit on the static wavefront error $\lVert{\boldsymbol\psi}\rVert \le 92$ nm rms \rachel{seems like it'd be good to link to some document where this is derived or set, if it lives somewhere public?}, set to achieve diffraction-limited imaging in $J$-band -- we can analytically search the space of possible static wavefront errors and find the maximum possible $|\partial\gamma_{{\rm obs},i}/\partial\psi_j|$ (units: nm$^{-1}$). This worst-case sensitivity can be used to set requirements on knowledge of the WFIRST wavefront. \rachel{For non-weak lensers, it may be necessary to spell out that the requirement fundamentally derives from the fact that we use the PSF model to correct the observed galaxy shapes for the convolution with the PSF, and if we do not know the wavefront well enough, our PSF model will be sufficiently wrong as to induce systematic uncertainty in the shear.  (This is obvious to us, but it's another point that I find gives rise to confusion.)}
...
\rachel{This paragraph makes the simplicity sound like a bug, but isn't a necessary feature at this point?  We do not know how to 100\% correct for some of the things you mentioned, so if we had included them, we wouldn't be able to uniquely isolate the impact of wavefront error.  It does mean we should eventually learn whether those other effects interact with this one, but there isn't a strong reason to think it does, right?}
...
5) The ratio of fluxes in each of the three galaxy components: a) de Vaucouleurs bulge, b) exponential disk, and c) random-walk star-forming knots (a maximum of 25\% of flux can exist in the knots \rachel{any constraint on ratio of disk to knot ratio?  e.g., I would not expect to have lots of galaxies with zero disk but 25\% knots}); 
...
For stars, we apply the SED of Alpha Lyra to a delta function model. \rachel{This seems like GalSim jargon; could we just say ``We model stars as point sources with the SED of Alpha Lyra''?}
...
Finally, each isolated postage stamp is processed through the steps described in Sec.~\ref{effects} to simulate the WFIRST observatory and detectors and written to disk. \rachel{Note that some detector effects depend on the overall flux level, so they will manifest differently in the SCA image and the postage stamps?} When all objects are added to the full SCA image, it is also processed through these steps and written to a FITS image file.
...
The model fit has 7 parameters: $e_{1,2}$, $p_{x,y}$, half-light radius, flux, and bulge flux fraction, where $e_{1,2}$ is the component of the ellipticity and $p_{x,y}$ is the pixel centroid offset. \rachel{So the two components are constrained to the same centroid, hlr, and shape?}
...
Some of the code in this module pre-dates this work \rachel{Should cite Arun's paper that presented this module}, but some of it was developed specifically for this project, especially updating some of the details to match Cycle 7 information, and to reflect new information from laboratory tests of persistence in WFIRST sensors.  
...
The PSF uses position-dependent (Zernike) aberrations, based on an investigation of the field-dependent wavefront errors for Cycle 5.0.6\footnote{\url{https://wfirst.gsfc.nasa.gov/science/sdt_public/wps/references/instrument/README_AFTA_C5_WFC_Zernike_and_Field_Data.pdf}}. \rachel{The code in the GalSim repo, galsim/wfirst/wfirst\_psfs.py, says it uses Cycle 7 results (see different file name listed in the code).}
...
\rachel{This subsubsection is somewhat bare in terms of references.  Arun's paper compiled a good set of refs that you might want to also refer to (at least 1-2 per effect).}
...
Shape measurement is then performed on the images with some PSF model bias, but using the fiducial PSF model for convolution \rachel{in the galaxy shape fitter?}, to simulate an unknown wavefront error.
...
\end{equation}
which leads to $A=\frac{\sigma^2}{\pi\nu_0}$. $\nu_0=\frac{1}{2\pi\tau}\approx 3.14\times10^{-4}$ Hz, with a time constant $\tau=1$ hr. \rachel{Is there some reference document that justifies this?}
...
\end{equation}
\rachel{In cases where there are PSF ellipticity variations across the simulations, people often write this as a number times PSF ellipticity, to address the fact that additive residuals often correlate with PSF ellipticity.  Did you consider this model, or rule out the need for it?  At least the PSF $e_1$ has some positive and negative values, so an additive shear bias would be partially obscured by just considering the ensemble average.}
...
Fig.~\ref{fig:f2pt_corrs} shows the two-point correlation function $\xi_{\pm}$ of the ellipticity difference in sky coordinates. \rachel{Should define what these correlation functions are.}
...
\caption{The changes of ellipticity with respect to changes in line of sight motion for jitter cases and wavefront error for the other modes.}\label{dpsi}
\begin{center}
\begin{tabular}{ lccc }
\hline
\hline
...
\end{center}
\caption[]{ The correlation function $\xi_+$ (red) and $\xi_-$ (blue) of ellipticity difference between each run, indicated by the label for each panel, and the \textsc{Fiducial} run. Negative points are shown as crosses. The $\xi_-$ points are offset slightly for clarity. \rachel{caption seems to have red and blue flipped?  Are we really getting anything out of the $\xi_-$ as shown?  If not, then perhaps omit, so it's easier to see what's happened in the more informative $\xi_+$?  (I think the conclusion re: $\xi_-$ could be OK if just stated in the text and it would make this plot easier to read.)}
\label{fig:f2pt_corrs}}
\end{figure}

\subsection{Comparison of analytic to numerical results}

The partial derivatives $|\partial e/\partial\psi|$ of the ellipticity with respect to wavefront should be less than $\lVert {\boldsymbol\Lambda} \rVert \lVert {\boldsymbol\psi} \rVert$ \rachel{$\Lambda$ has not been defined}, which is $8.98\times 10^{-4}\,$nm$^{-1}$ for the H158-band (this is an RSS of the two ellipticity components). For the partial derivatives of the ellipticity with respect to the second moments of the jitter pattern, $|\partial e/\partial\psi|$ should be less than $K_{\theta\theta}$ \rachel{this quantity has also not been defined}, which is $1.38\times 10^{-5}$\,mas$^{-2}$ for the H158-band. In the numerical results presented in Table~\ref{table:partials} \rachel{for some reason this is rendering as ``table 5.1'', which can't be right, so perhaps the label has been misassociated}, the largest partial derivatives are $4.5\times 10^{-4}\,$nm$^{-1}$ (for wavefront \rachel{errors?}) and $1.45\times 10^{-5}$\,mas$^{-2}$ (for jitter). For the wavefront drift case, this is consistent with the analytic expectations. For the anisotropic jitter case, the sensitivity $|\partial e/\partial\psi|$ determined from the simulations is $5\pm 2$\%\ larger than the analytic bound (we would expect a sensitivity equal to the analytic bound since the AniJitter run is the worst-case jitter pattern: the anisotropy is in the $e_1$ component for every exposure). The $5\pm 2$\% difference may simply represent the approximations made in the analytic calculation (e.g., no treatment of undersampling/image combination, a different shape measurement algorithm, etc.). \rachel{Are the analytic expectations tabulated somewhere that you could refer to, or could they even be put into the table for ease of comparison?}
...
The simulation framework described here is an iterative \rachel{In what sense is it iterative?}, but substantial step forward in the development of a significant synthetic WFIRST imaging survey, which incorporates a realistic set of photometric properties and distributions of galaxies and stars, complex morphological properties for galaxies, and most known detector non-idealities present in HgCdTe H4RG detectors. 
...
\rachel{All of this is true, but I wonder if you could tie more explicitly to the goals of the simulation framework?  For example, with a statement htat implementation of these effects in GalSim will enable studies of sensitivity to detector effects, placing requirements on our knowledge of those effects based on something beyond simple analytic prescriptions, etc.}
...
for the weak lensing (WL) \rachel{seems odd to define this acronym now after writing ``weak lensing'' throughout the entire main paper text?} program on the WFIRST mission, and documents the
...
WFIRST-IDRM studies, were for Gaussians). \rachel{acronyms have not been defined}
